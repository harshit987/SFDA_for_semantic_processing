...
if __name__ == "__main__":
    ...
    has_time = os.path.exists(os.path.join(input_dir, 'submission', 'time'))
    has_negation = os.path.exists(os.path.join(input_dir, 'submission', 'negation'))
    ...
    #     expected = path_lines(input_dir, 'practice_data', "**", to_system)
    # elif has_time:
    #     expected = path_lines(input_dir, 'practice_data', "time/**", to_system)
    # else:  # has_negation
    #     expected = path_lines(input_dir, 'practice_data', "**", to_system)
    # uploaded = path_lines(input_dir, 'submission', "**",to_system)
    ...
    metrics = {}
    if has_time:
        ref_domain = os.path.join(input_dir, 'true_labels', 'Test')
        res_domain = os.path.join(input_dir, 'submission', 'time')
        score_time(ref_domain, res_domain, metrics)
    if has_negation:
        ref_domain = os.path.join(input_dir, 'practice_data', 'negation/gold.tsv')
        res_domain = os.path.join(input_dir, 'submission', 'negation/system.tsv')
